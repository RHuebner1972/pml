...
```{r eval=TRUE,error=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
set.seed(01886)
...
The results from the decision tree show a predictive accuracy of `r round(cm$overall['Accuracy'], 4)*100`%, which is pretty good all things considered. Next, let's examine another approach to see if we can do better.
...
Using the random forest approach resulted in a `r round(confmat$overall['Accuracy'], 4)*100`% accuracy. This is a significant improvement over the decision tree approach. 
...
By using the decision tree algorithm, the predictive accuracy is `r round(cm$overall['Accuracy'], 4)*100`%.  Then, by using the random forest approach, the predictive accuracy is `r round(confmat$overall['Accuracy'], 4)*100`%. It is clear that the randomForest performs better for predictive accuracy.  There is an out-of-sample error of: `r oos`%.  
